          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className={`font-semibold text-sm px-4 py-2 rounded-full transition-colors relative ${
                  pathname === link.href ? "text-primary bg-blue-50" : "text-gray-700 hover:text-primary hover:bg-gray-50"
                }`}
              >
                {link.name}
              </Link>
            ...
            <div className="flex flex-col items-center gap-6 text-xl w-full px-6">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`font-bold w-full text-center py-3 border-b border-gray-100 ${
